...
API.interceptors.request.use((req) => {
  if (
    ...
    sessionStorage.getItem("reno-customer-token")
  ) {
    req.headers = {
      authorization: `Bearer ${customerToken}`,
      "reno-app-customer-auth-token": localStorage.getItem(
        "reno-customer-token"
      )
        ? JSON.parse(localStorage.getItem("reno-customer-token"))
        : JSON.parse(sessionStorage.getItem("reno-customer-token")),
...
const signInCustomerWithCode = (data) =>
  API.post(`/api/v1/customers/signinWithCode`, data);
const getTodayReceivedQuotes = () =>
  API.get(`/api/v1/quotes/getAllQuotesOfACustomerOfToday`);
const getReceivedQuotesPrevious = () =>
  API.get(`/api/v1/quotes/getAllQuotesOfACustomer`);
const denyAnyQuote = (quoteId) =>
  API.put(`/api/v1/quotes/quoteDenyByCustomer/${quoteId}`);
const sendResponseOnQuote = (quoteId, data) =>
...
const getRecentQuotesDeliveredToACustomer = () =>
  API.get(`/api/v1/quotes/getAllRecentQuotesDeliveredToACustomer`);
const deliveryConfirmationOfAQuote = (quoteId) =>
  API.put(`/api/v1/quotes/sendCustomerDeliveryResponse/${quoteId}`);
const getAllRecentQuotesForHomeScreen = () =>
  API.get(`/api/v1/quotes/getAllQuotesOfCustomerRecent`);
const getUpcomingPaymentsOfQuotes = () =>
...
const readAllNotificationsOfCustomer = () =>
  API.get(`/api/v1/notifications/readAllNotificationOfACustomer`);
const markNotificationsOfMerchantRead = (notificationId) =>
  API.put(
    `/api/v1/notifications/markNotificationOfCustomerAsRead/${notificationId}`
  );
const ApproveDisproveDelivery = (quoteId, status) =>
  API.put(`/api/v1/quotes/sendCustomerDeliveryResponse/${quoteId}/${status}`);
const getDashboardData = () =>
  API.get(`/api/v1/quotes/getAllFinancedAmtOfCustomer`);
const addCustomerNewTicket = (data) =>
  API.post(`/api/v1/issueTickets/adNewByCustomer`, data);
const getAllTicketsOfACustomer = () =>
  API.get(`/api/v1/issueTickets/getAllTicketsOfACustomer`);
const addNewTicketResponse = (data) =>
  API.post(`/api/v1/issueTicketsResponse/addNewResponseByCustomer`, data);
const getResponseOfAnyTicket = (ticketId) =>
  API.get(
    `/api/v1/issueTicketsResponse/getAllResponseOnAnyTicketByCustomer/${ticketId}`
  );
const checkUserPassword = (password) =>
  API.get(`/api/v1/customers/checkPassword/${password}`);
const updateUserPassword = (password) =>
  API.put(`/api/v1/customers/updatePassword/${password}`);
const getCustomerDetails = () =>
  API.get(`/api/v1/customers/getCustomerDetails`);
const updateCustomerPic = (data) =>
  API.put(`/api/v1/customers/updateProfilePic`, data);
const updateCustomerDetails = (data) =>
  API.put(`/api/v1/customers/updateCustomerProfile`, data);
const getSingleQuoteDetails = (quoteId) =>
  API.get(`/api/v1/quotes/getSingleQuoteDetails/${quoteId}`);
const makeNewPayment = (quoteId) =>
  API.post(
...
const verifyCodeForSignUp = (phoneNo, code) =>
  API.put(`/api/v1/customers/verifyCodeForSignUp/${phoneNo}/${code}`);
const removeUserForWrongCode = (phoneNo) =>
  API.delete(`/api/v1/customers/removeUserForWrongCode/${phoneNo}`);
const verifyCodeForSignIn = (user, code) =>
  API.put(`/api/v1/customers/verifyCodeForSignIn/${user}/${code}`);
