  ...
  const markStatusResolved = async (ticketId, status) => {
    ...
    if (data?.success === true) {
      ...
      if (isFound) {
        ...
        let newMyArr = newArr.filter((item) =>
          item._id == ticketId ? isFound : item
        );
        setAllData(newMyArr);
      }
    } else {
                      ...
                      <div className="d-flex flex-column">
                        ...
                        {item?.status == false ? (
                          <button
                            ...
                            onClick={() => markStatusResolved(item?._id, true)}
                          >
                            Mark As Resolved
                          </button>
                        ) : (
                          <button
                            ...
                            onClick={() => markStatusResolved(item?._id, false)}
                          >
                            Mark As Un-Resolved
                          </button>
                        )}
                        ...
                        >
                          ...
                          {item?.status == false ? (
                            ...
                            <p style={{ fontWeight: 600 }}>
                              Issus is Marked as Resolved By You, Please mark
                              this as Un-Resolved to start adding new responses.
                            </p>
                          )}
                        </div>
                      )}
