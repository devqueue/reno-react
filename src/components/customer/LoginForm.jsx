...
const LoginForm = () => {
  ...
  const sendMyCustEmail = async () => {
    ...
    if (data?.success === true) {
      toast.success(
        "Email Sent SuccessFully, Now please enter 4 digits PIN to reset your password which we have sent you in email."
      );
  ...
  const matchCode = async () => {
    ...
    if (data?.success === true) {
      ...
      localStorage.setItem(
        "reno-customerName",
        JSON.stringify(data?.User?.IdCardNo)
      );
      localStorage.setItem(
        "reno-customerPhoto",
        JSON.stringify(data?.User?.ProfilePic)
      );
      localStorage.setItem("reno-customerId", JSON.stringify(data?.User?.Id));
      navigate("/customer/dashboard/panel");
    } else {
  ...
  return (
    <div className="auth-container py-5">
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content p-3">
            <div class="modal-body">
              ...
              <input
                type="text"
                className="form-control fs-small text-dark"
                placeholder="Enter email here"
              />
              <button
                className="btn btn-success bg-darkBlue w-100 rounded-3 text-light mb-2"
          ...
          <div class="modal-content p-3">
            <div class="modal-body">
              ...
              <p className="text-secondary text-center mb-4 fs-small">
                ...
                Please check your email and choose a new password.
              </p>
              <div
                className="w-100 d-flex justify-content-center"
                data-bs-toggle="modal"
                data-bs-target="#forgotPasswordModal01"
              >
                <button
                  className="btn btn-info bg-darkBlue rounded-3 text-light mb-2 px-5"
                  data-bs-dismiss="modal"
                >
                  Okay
                </button>
      ...
      <div
        class="modal fade email-modal"
        id="forgotPasswordModal01"
        tabindex="-1"
        aria-labelledby="forgotPasswordModal01Label"
        aria-hidden="true"
      >
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content p-3">
          ...
          >
            <div className="modal-content p-3">
              <div className="modal-body">
                <div
                  className="d-flex justify-content-center max-w-100"
                ...
                {isMyFetching === true ? (
                  ...
                  <>
                    <button
                      className="btn btn-success bg-darkBlue w-100 rounded-3 text-light mb-2"
                      onClick={() => sendMyCustEmail()}
                    >
                      Send me PIN
                    </button>
                    <button
                      className="btn  btn-danger border modal-cancel border-color-darkBlue w-100 rounded-3 text-darkBlue "
                      onClick={handleSendMyEmailClose}
                    >
                      Cancel
                    </button>
                  </>
            ...
            <div className="modal-content p-3">
              <div className="modal-body">
                ...
                <input
                  ...
                  style={{ marginBottom: "15px" }}
                  value={custPasswordOne}
                  onChange={(e) => setCustPasswordOne(e.target.value)}
                  onBlur={(e) => checkPassword(e.target.value)}
                />
                ...
                <input
                  ...
                  style={{ marginBottom: "15px" }}
                  value={custPasswordTwo}
                  onChange={(e) => setCustPasswordTwo(e.target.value)}
                  onBlur={(e) => checkPassword(e.target.value)}
                />
                {isPasswordMatched === false && (
                  <span style={{ color: "crimson", fontSize: "12px" }}>
                ...
                {isMyFetching === true ? (
                  ...
                  <>
                    <button
                      className="btn btn-success bg-darkBlue w-100 rounded-3 text-light mb-2"
                      onClick={sendMyPassword}
                    >
                      Reset Now
                    </button>
                    <button
                      className="btn  btn-danger border modal-cancel border-color-darkBlue w-100 rounded-3 text-darkBlue "
                      onClick={handleEnterPasswordClose}
                    >
                      Cancel
                    </button>
                  </>
