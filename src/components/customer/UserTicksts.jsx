...
import { getAllNotificationsOfCustomer ,markNotificationsOfMerchantRead , addCustomerNewTicket, getAllTicketsOfACustomer , addNewTicketResponse , getResponseOfAnyTicket } from '../../api/CustomerApi'
...
const QuotesReceived = () => {
    ...
    const sendMyNewIssue = async () => {
        if(ticketData?.title == "" || ticketData?.desc == ""){
            ...
            setIsFetching(true)
            const {data} = await addCustomerNewTicket(ticketData);
            if(data?.success === true){
                toast.success("Issue Reported SuccessFully");
                const res = await getAllTicketsOfACustomer();
    ...
    useEffect(() =>{
        ...
        let userId = JSON.parse(sessionStorage.getItem("reno-customerId"));
        if(!userId){
            userId = JSON.parse(localStorage.getItem("reno-customerId"));
        }
        setUserId(userId)
    },[])
                                                            ...
                                                            <div className="row" key={item?._id} style={{paddingTop : '30px'}} >
                                                                ...
                                                                {/* Sending New Response */}
                                                                {
                                                                    item?.status == false ? (
                                                                        ...
                                                                        <p style={{fontWeight : 600}} >Issus is Marked as Resolved By Reno, You No Longer can Send Your Opinions to this. If You still unable to find out your issue, you generate a new Ticket Issue.</p>
                                                                    )
                                                                }
                                                            </div>
                                                        )
