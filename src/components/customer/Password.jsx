...
import {getAllNotificationsOfCustomer ,markNotificationsOfMerchantRead ,  checkUserPassword , updateUserPassword , updateCustomerPic , getCustomerDetails , updateCustomerDetails } from '../../api/CustomerApi'
...
const Password = () => {
    ...
    const updateMyData = async () => {
        ...
        if(isChanged == false){
            const {data} = await updateCustomerDetails(userDetails);
            if(data?.success === true){
                toast.success("Profile Details Updated SuccessFully")
                localStorage.setItem("reno-customerName", JSON.stringify(userDetails?.IDCardNo));
            }else{
                toast.error(data?.message)
            }
        }
